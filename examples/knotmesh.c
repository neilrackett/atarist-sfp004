...
/*
 * KNOTMESH.C - faceted (2,3) torus-knot mesh for Atari ST/STE/Mega STE
 *
 * The companion to torus.c's raymarcher: same trefoil, but as a low-poly
 * flat-shaded mesh (TSEG x RSEG facets, Three.js TorusKnotGeometry-style)
 * targeting real frame rates. The 68882 (via the atarist-sfp004 session
 * layer) runs the per-vertex rotate/project pipeline and per-face lighting
 * as fused coprocessor sessions; the 68000 does everything integer: backface
 * culling, depth-bucket painter's sort, and Bresenham-edged span fills
 * straight into the four bitplanes at full 320x200.
 *
 * Falls back to scalar soft-float transforms without the FPU (slow but
 * correct). On a Mega STE the 16 MHz clock + cache are enabled at startup.
 */
...
/* ---------- mesh density (the "segments" sliders) ---------- */
#ifndef TSEG
#define TSEG 26           /* tubular segments (along the knot)   */
#endif
#ifndef RSEG
#define RSEG 6            /* radial segments (around the tube)   */
#endif
#define NV (TSEG * RSEG)  /* vertices */
#define NF (TSEG * RSEG)  /* quad faces */

/* ---------- geometry (matches the raymarcher's knot) ---------- */
#define TORUS_R  1.0f
#define KNOT_W   0.35f
#define TUBE_R   0.28f  /* chunky, like the reference shot */
#define CAM_DIST 3.0f
#define PROJ_F   150.0f   /* focal: max |x'|/zc over the r=1.63 sphere at
                             CAM_DIST 3 is 0.648, so +/-98 px stays on screen */
...
/* ---------- palette: same cyan->white ramp as the raymarcher ---------- */
...
static Vec3 v_sub(Vec3 a, Vec3 b) {
    Vec3 r = { fp_sub(a.x, b.x), fp_sub(a.y, b.y), fp_sub(a.z, b.z) };
    return r;
}

static Vec3 v_cross(Vec3 a, Vec3 b) {
    Vec3 r = {
        fp_sub(fp_mul(a.y, b.z), fp_mul(a.z, b.y)),
        fp_sub(fp_mul(a.z, b.x), fp_mul(a.x, b.z)),
...
static Vec3 v_norm(Vec3 v) {
    float inv = fp_div(1.0f, fp_sqrt(v_dot(v, v)));
    Vec3 r = { fp_mul(v.x, inv), fp_mul(v.y, inv), fp_mul(v.z, inv) };
    return r;
}

/* ---------- mesh (built once at startup) ---------- */
static Vec3 overts[NV];               /* object-space vertices              */
static Vec3 fnorm[NF];                /* object-space face normals (unit)   */
static short fnorm_fix[NF][3];        /* the same normals in 8.8 fixed      */
static unsigned short fvert[NF][4];   /* quad vertex indices, fixed winding */

/* per-frame results */
static short vsx[NV], vsy[NV], vsz[NV];   /* screen x, y, depth (zc * 64)   */

/* ring offsets for RSEG = 6 (exact, no LUT rounding) */
static const float ring_c[6] = { 1.0f, 0.5f, -0.5f, -1.0f, -0.5f, 0.5f };
static const float ring_s[6] = { 0.0f, 0.8660254f, 0.8660254f,
                                 0.0f, -0.8660254f, -0.8660254f };

/*
 * Strand-centre curve of the same (2,3) knot the raymarcher fields:
 *   C(theta) = ((R + W cos(1.5 theta)) cos(theta),
 *                W sin(1.5 theta),
 *               (R + W cos(1.5 theta)) sin(theta)),  theta in [0, 4pi).
 * theta over two revolutions is 1024 LUT steps, psi = 1.5*theta is 1536;
 * both close exactly at i = TSEG. Tube rings sit in the plane spanned by
 * N (re-orthogonalised cross-section normal) and B = T x N.
 */
static void build_mesh(void)
{
    Vec3 curve[TSEG], cnorm[TSEG];
    int i, j;

    for (i = 0; i < TSEG; i++) {
        int thi = (i * 1024) / TSEG;
        int psi = (i * 1536) / TSEG;
        float cth = lut_cos(thi), sth = lut_sin(thi);
        float cps = lut_cos(psi), sps = lut_sin(psi);
        float wc  = fp_add(TORUS_R, fp_mul(KNOT_W, cps));
        curve[i].x = fp_mul(wc, cth);
        curve[i].y = fp_mul(KNOT_W, sps);
        curve[i].z = fp_mul(wc, sth);
        cnorm[i].x = fp_mul(cth, cps);   /* cross-section radial direction */
        cnorm[i].y = sps;
        cnorm[i].z = fp_mul(sth, cps);
    }

    for (i = 0; i < TSEG; i++) {
        Vec3 t = v_norm(v_sub(curve[(i + 1) % TSEG], curve[(i + TSEG - 1) % TSEG]));
        Vec3 b = v_norm(v_cross(t, cnorm[i]));
        Vec3 n = v_cross(b, t);          /* unit: b, t unit and orthogonal */
        for (j = 0; j < RSEG; j++) {
            Vec3 *v = &overts[i * RSEG + j];
            float rc = fp_mul(TUBE_R, ring_c[j]);
            float rs = fp_mul(TUBE_R, ring_s[j]);
            v->x = fp_add(curve[i].x, fp_add(fp_mul(n.x, rc), fp_mul(b.x, rs)));
            v->y = fp_add(curve[i].y, fp_add(fp_mul(n.y, rc), fp_mul(b.y, rs)));
            v->z = fp_add(curve[i].z, fp_add(fp_mul(n.z, rc), fp_mul(b.z, rs)));
        }
    }
    ...
    for (i = 0; i < TSEG; i++) {
        ...
        for (j = 0; j < RSEG; j++) {
            ...
            int f = i * RSEG + j;
            Vec3 e1, e2, n;
            fvert[f][0] = (unsigned short)(i  * RSEG + j);
            fvert[f][1] = (unsigned short)(i1 * RSEG + j);
            fvert[f][2] = (unsigned short)(i1 * RSEG + j1);
            fvert[f][3] = (unsigned short)(i  * RSEG + j1);
            /* geometric normal from the diagonals, oriented outward */
            e1 = v_sub(overts[fvert[f][2]], overts[fvert[f][0]]);
            e2 = v_sub(overts[fvert[f][3]], overts[fvert[f][1]]);
            n  = v_norm(v_cross(e1, e2));
            if (fbits(v_dot(n, v_sub(overts[fvert[f][0]], curve[i]))) < 0) {
                n.x = -n.x; n.y = -n.y; n.z = -n.z;
                /* flip winding too, so screen-space culling stays uniform */
                fvert[f][1] = (unsigned short)(i  * RSEG + j1);
                fvert[f][3] = (unsigned short)(i1 * RSEG + j);
            }
            fnorm[f] = n;
            fnorm_fix[f][0] = (short)(int)fp_mul(n.x, 256.0f);
            fnorm_fix[f][1] = (short)(int)fp_mul(n.y, 256.0f);
            fnorm_fix[f][2] = (short)(int)fp_mul(n.z, 256.0f);
        }
    }
}

/* ---------- per-frame rotation matrix (Rx * Ry, same feel as torus.c) ---------- */
static float mrot[9];
...
static void build_matrix(int ay, int ax)
{
    ...
    /* Ry: x' = cy x + sy z ; z' = -sy x + cy z. Rx: y' = cx y - sx z ; z' = sx y + cx z */
...
static void xform_vertex_fpu(const Vec3 *v, int idx)
{
    ...
    /* perspective: FP7 = PROJ_F / zc, scale x'/y', truncate to ints */
    ...
    /* depth = trunc(zc * 64) for the painter buckets */
    sfp004_cmd_in(C_MEM(FSGLMUL, 6), fu(64.0f));
    sfp004_cmd(C_REG(FINTRZ, 6, 6));
    vsz[idx] = (short)(long)sfp004_cmd_out(C_OUTL(6));
}
...
static void xform_vertex_soft(const Vec3 *v, int idx)
{
    ...
    vsz[idx] = (short)(int)fp_mul(zc, 64.0f);
}

/* ---------- per-face lighting, integer ---------- */
/* With 8.8 fixed-point normals a lighting dot product is three 68000 muls
 * (~15 us) - cheaper than even a fused coprocessor session (~70 us), and
 * identical with or without the FPU. The light is rotated into object space
 * once per frame, so face normals never transform at all. */
...
{
    /* L_obj = R^T * L (rows of R dotted as columns), then to 8.8 */
    Vec3 l;
...
static int face_colour(int f)
{
    ...
    long idx;
    if (d <= 0)
        return 2;                    /* facing away from the light */
    idx = 2 + ((d * 13L) >> 16);
    if (idx > 15) idx = 15;
    return (int)idx;
}

/* ---------- span rasteriser (integer only) ---------- */
static short span_min[SCREEN_H], span_max[SCREEN_H];

/* edge masks for both words of a plane pair, indexed by x&15 - variable
 * 32-bit shifts cost ~40 cycles each on a 68000, a lookup ~10 */
...
/* Per-scanline DDA over the edge: one 16.16 x step per line - for span
 * extraction only the x at each scanline matters, so walking every pixel
 * (Bresenham) is wasted work. Endpoints land exactly: the divide is by the
 * full height and the last line adds the remainder implicitly. */
...
/* Rasterise one face: DDA the edges into span_min/max, then fill spans
 * inline - patterns hoisted per face, row pointer walked incrementally,
 * and no clamps: vertices are provably on-screen, so spans are too. */
...
/* ---------- painter's sort: depth buckets, far-to-near ---------- */
#define NBUCKET 256
static short bucket_head[NBUCKET];
static short face_next[NF];

/* ---------- screen plumbing (as torus.c) ---------- */
static void clear_screen(unsigned char *scr) { memset(scr, 0, SCREEN_BYTES); }

/* ---------- blitter clear (shared mode, overlaps the cull/sort) ---------- */
...
static int have_blitter = 0;   /* Blitmode() inquiry, latched in main() */

/* OP 0 (dest = all zeros) reads no source: a pure fill. Hog mode: shared
 * mode ran the clear at half speed and the sort phase was too short to
 * hide it (measured), so just take the bus and finish fast. */
static void blit_clear_start(unsigned char *scr)
{
    BLT_EM1 = 0xFFFF; BLT_EM2 = 0xFFFF; BLT_EM3 = 0xFFFF;
    BLT_DXI = 2; BLT_DYI = 2;
    BLT_DADR = (unsigned long)scr;
    BLT_XCNT = PLANE_WORDS * PLANES;   /* one scanline of words per line */
    BLT_YCNT = SCREEN_H;
    BLT_HOP = 0; BLT_OP = 0; BLT_SKEW = 0;
    BLT_CTRL = 0xC0;                   /* busy + hog: run to completion */
}

static void blit_clear_wait(void)
{
    while (BLT_CTRL & 0x80)
        ;
}
...
/* ---------- ms/frame readout (physical pixels, as torus.c) ---------- */
...
/* ---------- render one frame ---------- */
static unsigned long xform_ms;   /* vertex-pipeline share of the frame */
static unsigned long paint_ms;   /* lighting + span-fill share          */
...
static void render_frame(unsigned char *scr, int angle_y, int angle_x)
{
    ...
    /* cull + depth-bucket the visible faces (integer only) */
    for (b = 0; b < NBUCKET; b++) bucket_head[b] = -1;
    for (f = 0; f < NF; f++) {
        ...
        depth = vsz[v0] + vsz[v1] + vsz[v2] + vsz[fvert[f][3]];
        depth >>= 4;                      /* zc*64 x4 summed: /16 -> 0..291 */
        if (depth < 0) depth = 0;
        if (depth > NBUCKET - 1) depth = NBUCKET - 1;
        face_next[f] = bucket_head[depth];
        bucket_head[depth] = (short)f;
    }

    if (have_blitter) {
        blit_clear_start(scr);
        blit_clear_wait();
    } else {
        clear_screen(scr);
    }
...
#endif
    /* painter: far (large zc) first */
    for (b = NBUCKET - 1; b >= 0; b--) {
        short f2 = bucket_head[b];
        while (f2 >= 0) {
            fill_quad(scr, f2, face_colour(f2));
            f2 = face_next[f2];
        }
    }
#ifndef SFP004_MOCK
...
int main(void)
{
    ...
    *((volatile unsigned short *)0xFF8260L) = 0;
    ...
    while (1) {
        ...
        draw_ms(draw, paint_ms, 18);               /* lighting + fills    */
